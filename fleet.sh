...
grouped = set()
for g in ("bastion", "dev", "gw_node"):
    grouped |= set(d.get(g, {}).get("hosts", []))
print("\n".join(sorted(allh - grouped)))
' 2>/dev/null)" || return 0
  if [ -n "$stragglers" ]; then
    echo "fleet.sh: WARNING — these hosts are in no profile group (bastion/dev/gw_node)" >&2
    echo "          and will get the default (= bastion) behaviour, public SSH:" >&2
    printf '            %s\n' $stragglers >&2
    echo >&2
  fi
}
