...
| **flutter** | *(flutter profile only)* The postmodern web client (static Flutter build via nginx) as a pinned GHCR container (`Network=host`, public `:8080`, `/healthz`). No runtime config/secrets except the GHCR pull token. **Backend URLs are baked at image build** (`ACCOUNT_URL`/`CHAT_URL` build-args), not set here. Runtime contract: `postmodern-flutter/CONTAINER.md`. |
