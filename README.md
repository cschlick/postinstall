...
Connections are plain SSH as **pmuser** (passwordless sudo — no `-K` needed):
the control machine just needs pmuser's private key. gw_nodes are listed by
...
| **accounts** | Ensures pmuser exists (full passwordless sudo, ed25519 keypair, emergency console password from `pmuser_password_hash`); fully purges `linuxuser`. |
...
| **seed** | *(seed profile only)* One-shot demo-world seeder (`postmodern-server/Containerfile.seed`) as a podman quadlet that is **not** auto-started — the operator triggers `systemctl start pm-seed`. `mesh_client` for the mTLS chat DB; targets the planes over `ws://`. Secrets from ansible-vault. |
| **flutter** | *(flutter profile only)* The postmodern web client (static Flutter build via nginx) as a pinned GHCR container (`Network=host`, public `:8080`, `/healthz`). Backend URLs are fed at deploy via a bind-mounted `config.json` (one image, any stack), not baked. Only secret is the GHCR pull token. Runtime contract: `postmodern-flutter/CONTAINER.md`. |
| **imagemod** | *(imagemod profile only)* The postmodern image-moderation ML service (PyTorch NSFW/NudeNet/InsightFace/age models) as a pinned GHCR container (`Network=host`, mesh-only `:8000`, `/health`). The chat plane calls it over the overlay. Model weights persist in a host cache dir; the Bearer `API_KEY` comes from `vault-imagemod.yml`. Runtime contract: `postmodern-imagemod/API.md`. |
